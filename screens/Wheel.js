...
import Wheell from '../Utils/Wheel';
...
const Wheel = ({ navigation }) => {
  ...
  const getdata = ({ brandName, modelName, fuelType }) => {
    ...
    fetch("https://digi-servizzy-backend.herokuapp.com/api/get-tyre-pack", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        brandName: brandName,
      ...
      .then((data) => {
        ...
        const filterdata = data.data.filter((x) => Object.keys(Wheell).includes(x.title))
