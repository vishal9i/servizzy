...
const Batterypack = ({ navigation }) => {
  ...
  const getdata = ({ brandName, modelName, fuelType }) => {
    ...
    fetch("https://digi-servizzy-backend.herokuapp.com/api/get-battery-pack", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        brandName: brandName,
      ...
      .then((data) => {
        ...
        const filterdata = data.data.filter((x) => Object.keys(Battery).includes(x.title))
