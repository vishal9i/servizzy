...
import Electric from '../Utils/Electric';
...
const Electrical = ({ navigation }) => {
  ...
  // https://digi-servizzy-backend.herokuapp.com/api/add-electricity-servic-pac
  const getdata = ({ brandName, modelName, fuelType }) => {
    console.log(brandName, modelName, fuelType);
    fetch("https://digi-servizzy-backend.herokuapp.com/api/get-electricity-pack", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        brandName: brandName,
      ...
      .then((data) => {
        ...
        const filterdata = data.data.filter((x) => Object.keys(Electric).includes(x.title))
