...
import Insurance from '../Utils/Insurance';
...
const InsuranceClaim = ({ navigation }) => {
  ...
  const getdata = ({ brandName, modelName, fuelType }) => {
    ...
    fetch("https://digi-servizzy-backend.herokuapp.com/api/get-insurence-pack", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        brandName: brandName,
      ...
      .then((data) => {
        ...
        const filterdata = data.data.filter((x) => Object.keys(Insurance).includes(x.title))
